codeunit 81190 "MICA Flow Extract Shipped Conf"
{
    ...
    var
        ...
        BufferImpMICAShippedConfirmation: XmlPort "MICA Shipped Confirmation";
        InStream: InStream;
    ...
    begin
        ...
        Clear(BufferImpMICAShippedConfirmation);
        BufferImpMICAShippedConfirmation.SetSource(InStream);
        BufferImpMICAShippedConfirmation.SetFlowEntry(Rec);
        Commit();
        if not BufferImpMICAShippedConfirmation.Import() then
            Rec.AddInformation(MICAFlowInformation."Info Type"::Error, ImportAbortedLbl, StrSubstNo(AbortedAfterLbl, GetLastErrorText()))
        else begin
            BufferImpMICAShippedConfirmation.GetBuffer(MICAFlowBuffShippedConfirm2);
            Rec.AddInformation(MICAFlowInformation."Info Type"::Information, StrSubstNo(ImportFinishedLbl, BufferImpMICAShippedConfirmation.GetRecordCount()), '');
            Rec.Validate("Receive Status", Rec."Receive Status"::Loaded);
            Rec.Validate("Buffer Count", BufferImpMICAShippedConfirmation.GetRecordCount());
        end;
