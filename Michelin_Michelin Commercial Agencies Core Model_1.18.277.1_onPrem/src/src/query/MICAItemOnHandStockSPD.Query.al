query 82341 "MICA Item OnHand Stock SPD"
{
    QueryType = Normal;
            ...
            dataitem(Item_Ledger_Entry; "Item Ledger Entry")
            {
                ...
                dataitem(Location; Location)
                {
                    ...
                    DataItemTableFilter = "MICA DRP IN Location Code" = filter(<> ''), "MICA Entry Type" = const("P (Main)"),
                                            "Use As In-Transit" = const(false), "MICA SPD OnHand Qty. Code" = filter(<> ''),
                                            "MICA Exclude from DRP" = const(false);

                    column(MICA_SPD_OnHand_Qty__Code; "MICA SPD OnHand Qty. Code")
                    {
