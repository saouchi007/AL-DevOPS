report 82760 "MICA Sugg. Price Recalc. Wksh."
{
    Caption = 'Suggest Price Recalculation Worksheet';
    ProcessingOnly = true;
    UseRequestPage = true;
    UsageCategory = None;
    ...
    dataset
    {
        dataitem("Sales Line"; "Sales Line")
        {
            ...
            var
                SalesPriceAmount: Decimal;
            begin
                Clear(MICASuggImpPricRebChng);

                if not MICASuggImpPricRebChng.CheckSalesOrderDateForSuggestion("Sales Line"."Document No.", SuggestionFilterOrderDate) then
                    exit;

                if not MICASuggImpPricRebChng.CheckLinesStatusReserveOnHandInTransit("Sales Line", SuggestionFilterDate) then
                    exit;

                if MICASuggImpPricRebChng.FindSalesPrice("Sales Line", SalesPriceAmount) then
                    if SalesPriceAmount <> "Unit Price" then
                        MICASuggImpPricRebChng.CheckAndFillPriceRecalcWorksheet("Sales Line", SalesPriceAmount);
            end;
        }
    }
                ...
                group(Options)
                {
                    ...
                    group("Copy to Price Recalc. Worksheet")
                    {
                        ...
                        field(SalesStatus; ToSalesStatus)
                        {
                            ...
                            trigger OnLookup(var Text: Text): Boolean
                            begin
                                MICASuggImpPricRebChng.CheckAndProcessSalesStatus(ToSalesStatus, 0);
                            end;
                        }
                    }
                }
    ...
    begin
        ...
        SuggestionFilterOrderDate := CalcDate('<-' + Format(SalesReceivablesSetup."MICA SO Price Rec. Excl. Wind.") + '>', WorkDate());
    end;
    ...
    var
        ...
        SuggestionFilterOrderDate: Date;
        ToSalesStatus: Text;
        ToSalesCode: Code[20];
        [InDataSet]
