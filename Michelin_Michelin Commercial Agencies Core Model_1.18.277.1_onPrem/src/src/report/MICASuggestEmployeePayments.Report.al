report 82141 "MICA Suggest Employee Payments"
{
    ...
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            RequestFilterFields = "No.", "Employee Posting Group", "MICA Payment Method Code";
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                ...
                {
                    Caption = 'Options';
                    field(SetDefaultValue; SetDefaultValueValue)
                    {
                        ApplicationArea = All;
                    ...
                    group("Summarize Results")
                    {
                        ...
                        field(SummarizePerDimText; SummarizePerDimTextValue)
                        {
                            ...
                            begin
                                DimSelectionBuf.SetDimSelectionMultiple(3, REPORT::"Suggest Employee Payments", SummarizePerDimTextValue);
                            end;
                        }
                    }
    ...
    var
        ...
        TempOldTempEmplPaymentBuffer: Record "Employee Payment Buffer" temporary;
    ...
    begin
        with GenJnlLine do begin
            Init();
            Validate("Posting Date", PostingDateValue);
        ...
        if TempPayableEmployeeLedgerEntry.Find('-') then
            repeat
                ...
                repeat
                    ...
                    Employee.Get(TempPayableEmployeeLedgerEntry."Employee No.");
                    ...
                    TempEmplPaymentBuffer."Payment Method Code" := Employee."MICA Payment Method Code";
    ...
    begin
        Clear(TempOldTempEmplPaymentBuffer);
                ...
                with GenJnlLine do begin
                    ...
                    if SummarizePerEmpl then
                        ...
                        if DocNoPerLine then begin
                            ...
                            if (TempEmplPaymentBuffer."Employee No." = TempOldTempEmplPaymentBuffer."Employee No.") and
                               (TempEmplPaymentBuffer."Currency Code" = TempOldTempEmplPaymentBuffer."Currency Code")
                            then
                                "Document No." := TempOldTempEmplPaymentBuffer."Document No."
                            else begin
                                "Document No." := NextDocNo;
                                NextDocNo := IncStr(NextDocNo);
                                TempOldTempEmplPaymentBuffer := TempEmplPaymentBuffer;
                                TempOldTempEmplPaymentBuffer."Document No." := "Document No.";
                            end;
                    "Account Type" := "Account Type"::Employee;
                    ...
                    if MICAFinancialReportingSetup.Get() then
                        if SetDefaultValueValue then
                            if "Amount (LCY)" < MICAFinancialReportingSetup."Mass Payment Amount" then
                                Validate("Payment Method Code", MICAFinancialReportingSetup."Dynamic Pay. Mtd. Code Value 1")
                            else
                                Validate("Payment Method Code", MICAFinancialReportingSetup."Dynamic Pay. Mtd. Code Value 2");
                    Insert();
    ...
    var
        BankAccLocal: Record "Bank Account";
    begin
        if BalAccType = BalAccType::"Bank Account" then
            if BalAccNo <> '' then begin
                BankAccLocal.Get(BalAccNo);
                if BankAccLocal."Currency Code" <> '' then
