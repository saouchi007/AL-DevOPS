xmlport 82022 "MICA Supplier Inv.FunctionlACK"
{    //INTGIS001 – Supplier Invoice Integration
    ...
    begin
        ...
        SUBSECOND := '0000';
        RexTimeZone.Setrange(ID, MySessionSettings.TimeZone());
        RexTimeZone.FindFirst();
        TIMEZONE := DelChr(CopyStr(RexTimeZone."Display Name", StrPos(RexTimeZone."Display Name", 'UTC') + 3, 6), '=', ':');
    ...
    procedure SetParameters(var InMICAFlowEntry: Record "MICA Flow Entry"; InPurchaseHeader: Record "Purchase Header"; var inMICAFlowBufferSupplierInv: Record "MICA Flow Buffer Supplier Inv.")
    begin
        MICAFlowEntry := InMICAFlowEntry;
        PurchaseHeader := InPurchaseHeader;
        MICAFlowBufferSupplierInv := inMICAFlowBufferSupplierInv;
        MICAFlowBufferSupplierInv.GetErrorText(ErrorCodes, ErrorDescription);
    end;
}
